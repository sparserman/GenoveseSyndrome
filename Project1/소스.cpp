...
const TCHAR* DiaryText2 = TEXT("5월 2일\n"
	"친구들이 담배를 피며 나에게 담배 1대를 권하길래 담배는 좋지 않은 것 같다고 말을 꺼냈다가 남자 주제에 패기가 없다고 놀림을 받았다."
...
void BackGround(HDC hdc)
{
	...
	if (stage == 2)
	{
		DrawBitmap(hdc, 0, 0, BokDoBit[1]);		// 복도2
	}
	if (stage == 3)
	{
	...
	if (stage == 4)
	{
		DrawBitmap(hdc, 0, 0, BokDoBit[3]);		// 복도 4
	}
	if (stage == 5)
	{
...
void CloseDoor(HDC hdc)
{
	...
	case 0:
		...
		break;
	case 1:
		if (stage == 1 && y < 190 && x > 920 && x < 1100)		//창고
		{
			page = 100;
		}
		else
		...
		break;
	case 2:
		if (y < 190 && x > 50 && x < 120 && stage == 4 && Guard >= 1)			// 교무실
		{
			SelectButton = 1;
		...
		else if (stage == 5 && y > 435 && x < 600 && Class4Door == 0 && Guard >= 2)		// 1-4반
		{
			SelectButton = 1;
		...
		else if (stage == 4 && y < 190 && x > 930 && x < 1010 && BedDoor == 0 && Guard >= 3)		// 숙직실
		{
			SelectButton = 1;
		...
		else if (stage == 3 && y < 190 && x > 930 && x < 1010 && HealDoor == 0 && Guard >= 4)		// 보건실
		{
			SelectButton = 1;
	...
	case 4:
		...
		if (hint == 2 && Dap == 0  && y < 190 && x > 50 && x < 120 && stage == 4)
		{
			name = TEXT("이도현");
			text = TEXT("두번째 일기장에 힌트가 있을거같은데..");
			FontSet(hdc, grt, nrt, text, name, ttext, P_IllBit, 0, FONTSIZE, RGB(0, 0, 0), 0, 0);
		}
		else if (hint == 2 && Dap == 0)
		{
			name = TEXT("이도현");
	...
	case 0:
		...
		else if (Class1Door == 1 && stage == 11)
		{
			...
			if (Sound == 0 && BGMCheck >= 1)
			{
				...
				openBgm.lpstrDeviceType = "mpegvideo";// wave , "mpegvideo"; //mp3 형식
				openBgm.lpstrElementName = "In Beautiful Landscape.mp3";
				mciSendCommand(0, MCI_OPEN, MCI_OPEN_ELEMENT | MCI_OPEN_TYPE, (DWORD)(LPVOID)&openBgm);
				dwID = openBgm.wDeviceID;
				mciSendCommand(dwID, MCI_PLAY, MCI_DGV_PLAY_REPEAT, (DWORD)(LPVOID)&openBgm);
				BGMCheck = 2;
		...
		else if (BGMCheck >= 1 && Class1Door == 2)
		{
			if (Sound == 0 && BGMCheck >= 1)
			{
				mciSendCommand(dwID, MCI_CLOSE, 0, NULL);//정지 및 메모리에서 삭제
...
void Box(HDC hdc)
{
	...
	case 1:
		...
		{
			name = TEXT("시스템");
			text = TEXT("이수현 - xx 회사 대리, 8시에 출발하여 9시에 회사 도착함 주로 10시에 퇴근함");
			ttext = TEXT("\n          (가장 쉬움)");
...
void EndPiano(HDC hdc)
{
	...
	case 1:
		...
		{
			name = TEXT("시스템");
			text = TEXT("(숫자키 1~8로 피아노를 칠 수 있습니다, 'I' 키로 피아노를 사용 중에 악보를 킬 수 있습니다.)");
			FontSet(hdc, grt, nrt, text, name, ttext, System, 0, FONTSIZE, RGB(0, 0, 0), 0, 0);
		}
		break;
	case 2:
...
void End(HDC hdc)
{
	...
	if (Endingy < -3000)
	{
		...
		if (Sound == 2 && BGMCheck >= 1 && NPCDie == 0 && Class1Door != 2)
		{
			openBgm.lpstrDeviceType = "mpegvideo";// wave , "mpegvideo"; //mp3 형식
			openBgm.lpstrElementName = "Kaine.mp3";
			mciSendCommand(0, MCI_OPEN, MCI_OPEN_ELEMENT | MCI_OPEN_TYPE, (DWORD)(LPVOID)&openBgm);
			dwID = openBgm.wDeviceID;
			mciSendCommand(dwID, MCI_PLAY, MCI_DGV_PLAY_REPEAT, (DWORD)(LPVOID)&openBgm);
			BGMCheck = 2;
		...
		else if (Class1Door == 2 && Sound == 2 && BGMCheck >= 1)
		{
			openBgm.lpstrDeviceType = "mpegvideo";// wave , "mpegvideo"; //mp3 형식
			openBgm.lpstrElementName = "In Beautiful Landscape.mp3";
			mciSendCommand(0, MCI_OPEN, MCI_OPEN_ELEMENT | MCI_OPEN_TYPE, (DWORD)(LPVOID)&openBgm);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_UP:
			...
			if (Titleint && TitleSelect != 1 && page != 2 && OptionCheck != 1)
			{
				TitleSelect--;
			}
			break;
		case VK_DOWN:
			...
			if (Titleint && TitleSelect != 3 && OptionCheck != 1)
			{
				TitleSelect++;
			}
			break;
